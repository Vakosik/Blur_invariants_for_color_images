import numpy as np
from blur_invariants import central_moments
from blur_invariants import blur_invariants
from joblib import Parallel, delayed
from tqdm import tqdm


def compute_img_n_temp_invariants(img, temps, temp_sz, order, img_name, method, combs, img_invariants,
    ...
    invs_counts = []
    if "centrosym_invs" in method:
        N = 2
    elif "unconstrained_invs" in method:
        N = 1
    for comb in combs:
...
def get_template_invariants(temps, invs_counts, order, N, combs, typennum):
    n_temp = len(temps)
